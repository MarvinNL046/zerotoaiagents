...
import { getAgentsByCategory, type AiAgentProvider } from "@/lib/ai-agent-data";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "codingAgents.meta" });

  return {
    title: t("title"),
    description: t("description"),
    alternates: {
      canonical: `${baseUrl}${locale === "en" ? "" : `/${locale}`}/best/coding-agents`,
    },
  };
}

export default async function CodingAgentsPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);

  const t = await getTranslations({ locale, namespace: "codingAgents" });
  const agents = getAgentsByCategory("coding-agents");

  // Get top 3 for quick picks
  const [bestOverall, bestValue, mostPopular] = [
    agents.find((a) => a.slug === "claude-code"),
    agents.find((a) => a.slug === "github-copilot"),
    agents.find((a) => a.slug === "cursor"),
  ];
      ...
      <div className="flex flex-col">
        ...
        <section className="relative py-16 lg:py-24 overflow-hidden">
          ...
          <div className="container relative">
            <BreadcrumbSchema
              items={[{ name: "Best Coding Agents", href: "/best/coding-agents" }]}
              className="mb-6"
            />
          ...
          <div className="container">
            <div className="max-w-3xl mx-auto text-center space-y-6">
              <h2 className="text-3xl md:text-4xl font-bold">
                Find Your Perfect Coding Agent
              </h2>
