...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "noCodeAgents.meta" });

  return {
    title: t("title"),
    description: t("description"),
    alternates: {
      canonical: `${baseUrl}${locale === "en" ? "" : `/${locale}`}/best/no-code-agents`,
    },
  };
}

export default async function NoCodeAgentsPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);

  const t = await getTranslations({ locale, namespace: "noCodeAgents" });
  const agents = getAgentsByCategory("no-code-builders");

  const [bestOverall, bestValue, mostPopular] = [
    agents.find((a) => a.slug === "n8n-ai"),
    agents.find((a) => a.slug === "make-ai"),
    agents.find((a) => a.slug === "zapier-central"),
  ];
      ...
      <div className="flex flex-col">
        ...
        <section className="relative py-16 lg:py-24 overflow-hidden">
          ...
          <div className="container relative">
            <BreadcrumbSchema
              items={[{ name: "Best No-Code AI Agent Builders", href: "/best/no-code-agents" }]}
              className="mb-6"
            />
          ...
          <div className="container">
            <div className="max-w-3xl mx-auto text-center space-y-6">
              <h2 className="text-3xl md:text-4xl font-bold">
                Find Your Perfect No-Code Builder
              </h2>
