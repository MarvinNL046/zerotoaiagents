...
import { Building, Clock, ArrowRight, Crown, TrendingUp, Zap } from "lucide-react";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "enterpriseAgents.meta" });

  return {
    title: t("title"),
    description: t("description"),
    alternates: {
      canonical: `${baseUrl}${locale === "en" ? "" : `/${locale}`}/best/enterprise-agents`,
    },
  };
}

export default async function EnterpriseAgentsPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);

  const t = await getTranslations({ locale, namespace: "enterpriseAgents" });
  const agents = getAgentsByCategory("enterprise");

  const [bestOverall, bestValue, mostPopular] = [
    agents.find((a) => a.slug === "microsoft-copilot-studio"),
    agents.find((a) => a.slug === "salesforce-agentforce"),
    agents.find((a) => a.slug === "google-vertex-ai-agent-builder"),
  ];
      ...
      <div className="flex flex-col">
        <section className="relative py-16 lg:py-24 overflow-hidden">
          ...
          <div className="container relative">
            <BreadcrumbSchema
              items={[{ name: "Best Enterprise AI Platforms", href: "/best/enterprise-agents" }]}
              className="mb-6"
            />
                ...
                <Card className="relative border-2 border-green-500/50 bg-gradient-to-b from-green-500/5 to-transparent">
                  ...
                  <CardContent className="pt-8 text-center space-y-4">
                    ...
                    <div className="text-3xl font-bold text-primary">
                      ...
                      <span className="text-sm font-normal text-muted-foreground">/conversation</span>
                    </div>
                    <AffiliateButton
              ...
              {mostPopular && (
                <Card className="relative border-2 border-blue-500/50 bg-gradient-to-b from-blue-500/5 to-transparent">
                  ...
                  <CardContent className="pt-8 text-center space-y-4">
                    ...
                    <div className="text-3xl font-bold text-primary">
                      Pay-as-you-go
                      <span className="text-sm font-normal text-muted-foreground block">pricing</span>
                    </div>
                    <AffiliateButton
        ...
        <section className="py-16 lg:py-24">
          <div className="container">
            <div className="max-w-3xl mx-auto text-center space-y-6">
              <h2 className="text-3xl md:text-4xl font-bold">Find Your Enterprise AI Platform</h2>
