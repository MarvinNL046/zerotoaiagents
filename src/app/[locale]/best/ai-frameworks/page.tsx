...
import { Wrench, Clock, ArrowRight, Crown, TrendingUp, Zap } from "lucide-react";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "aiFrameworks.meta" });

  return {
    title: t("title"),
    description: t("description"),
    alternates: {
      canonical: `${baseUrl}${locale === "en" ? "" : `/${locale}`}/best/ai-frameworks`,
    },
  };
}

export default async function AiFrameworksPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);

  const t = await getTranslations({ locale, namespace: "aiFrameworks" });
  const agents = getAgentsByCategory("frameworks");

  const [bestOverall, bestValue, mostPopular] = [
    agents.find((a) => a.slug === "crewai"),
    agents.find((a) => a.slug === "langgraph"),
    agents.find((a) => a.slug === "autogen"),
  ];
      ...
      <div className="flex flex-col">
        <section className="relative py-16 lg:py-24 overflow-hidden">
          ...
          <div className="container relative">
            <BreadcrumbSchema
              items={[{ name: "Best AI Agent Frameworks", href: "/best/ai-frameworks" }]}
              className="mb-6"
            />
                ...
                <Card className="relative border-2 border-yellow-500/50 bg-gradient-to-b from-yellow-500/5 to-transparent">
                  ...
                  <CardContent className="pt-8 text-center space-y-4">
                    ...
                    <div className="text-3xl font-bold text-primary">
                      Free
                      <span className="text-sm font-normal text-muted-foreground block">Open Source</span>
                    </div>
                    <AffiliateButton
        ...
        <section className="py-16 lg:py-24">
          <div className="container">
            <div className="max-w-3xl mx-auto text-center space-y-6">
              <h2 className="text-3xl md:text-4xl font-bold">Start Building with Frameworks</h2>
