...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "supportAgents.meta" });

  return {
    title: t("title"),
    description: t("description"),
    alternates: {
      canonical: `${baseUrl}${locale === "en" ? "" : `/${locale}`}/best/customer-support-agents`,
    },
  };
}

export default async function CustomerSupportAgentsPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);

  const t = await getTranslations({ locale, namespace: "supportAgents" });
  const agents = getAgentsByCategory("customer-support");

  const [bestOverall, bestValue, mostPopular] = [
    agents.find((a) => a.slug === "intercom-fin"),
    agents.find((a) => a.slug === "zendesk-ai"),
    agents.find((a) => a.slug === "ada-ai"),
  ];
      ...
      <div className="flex flex-col">
        <section className="relative py-16 lg:py-24 overflow-hidden">
          ...
          <div className="container relative">
            <BreadcrumbSchema
              items={[{ name: "Best Customer Support Agents", href: "/best/customer-support-agents" }]}
              className="mb-6"
            />
                ...
                <Card className="relative border-2 border-yellow-500/50 bg-gradient-to-b from-yellow-500/5 to-transparent">
                  ...
                  <CardContent className="pt-8 text-center space-y-4">
                    ...
                    <div className="text-3xl font-bold text-primary">
                      ...
                      <span className="text-sm font-normal text-muted-foreground">/resolution</span>
                    </div>
                    <AffiliateButton
          ...
          <div className="container">
            <div className="max-w-3xl mx-auto text-center space-y-6">
              <h2 className="text-3xl md:text-4xl font-bold">
                Find Your Perfect Support Agent
              </h2>
